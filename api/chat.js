...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Sistem Anti-Spam (Rate Limit)
    try {
      const clientIp = req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown_ip';
      const safeIpId = String(clientIp).replace(/\./g, '_').replace(/:/g, '_'); 
      ...
      console.error('Firestore Rate Limit Error (Skipped):', fsErr);
    }
